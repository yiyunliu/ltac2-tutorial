...
(** Now we can define an Ltac2 integer named [two] *)
Ltac2 two := 1.

(** Note that [two] lives in a different namespace from Gallina terms, which are terms we construct
 through [Definition] or [Lemma] commands without the [Ltac2] prefix. Attempting to use [two] in
 a [Definition] results in an error *)

Fail Definition three := two + 1.

(** The error message complains that [two] is not in scope *)

(** Ltac2 terms and Gallina terms live in different namespaces. We can define a Gallina term [two]
 as follows. *)
Definition two := 2.

(** The [Ltac2 Check] command allows us to query the type of the Ltac2 definition of [two], which is
 an Ltac2 integer [Init.int]. *)
Ltac2 Check two.

(** The [Check] command, on the other hand, queries the type of a Gallina term. Invoking it on [two]
 gives us [nat], the type of the Gallina term [two]  *)
Check two.

(** Similarly, the [Ltac2 Eval] and [Compute] commands allow us to run Ltac2
 and Gallina programs respectively. *)
Ltac2 Eval two.

Compute two.

(** The syntax of the Ltac2 allows us to disambiguate between the metavariables from Ltac2 and
 the variables from Gallina. In the following definition, we construct  *)
